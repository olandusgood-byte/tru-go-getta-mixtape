...
6. Re-test cleanly after the rollback cutoff.
7. Record a release checkpoint. Checkpoint keys are unique per staging record and may include creator attribution.
8. A clean `pass` plus at least one checkpoint advances the staging record to `passed` and marks the associated release candidate `ready` when applicable.
9. Production advancement is intentionally not automated by AB-005 and remains separately gated.
...
- Clean-pass evaluation is isolated to smoke checks after the stored rollback cutoff when a rollback occurred.
- Any `fail`, `blocked`, or `manual` result in the clean attempt prevents advancement.
