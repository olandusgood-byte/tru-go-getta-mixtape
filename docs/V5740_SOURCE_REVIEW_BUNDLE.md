...
candidate output, candidate lint, candidate XML validation, and a consolidated
review summary in one artifact bundle.
...
`READY_FOR_HUMAN_REVIEW` means only that source intake, source XML parsing,
candidate generation, candidate XML parsing, and regression lint passed. It
never means deployed, production verified, or human approved.
