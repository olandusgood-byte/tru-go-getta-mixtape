(() => {
  ...
  window.TGGCareer={career,record(){career.recordings++;rep(25);notify('TRACK RECORDED +25 REP')},mixtape(){if(career.recordings<3){notify('RECORD 3 TRACKS FIRST');return}career.mixtapes++;rep(75);notify('MIXTAPE RELEASED +75 REP')},upgrade(){const cost=250*career.studioLevel;const s=window.TGGGame?.getState?.();if(!s||s.cash<cost){notify('NOT ENOUGH CASH — $'+cost+' NEEDED');return}s.cash-=cost;career.upgrades++;rep(10);if(window.TGGGame?.refresh)window.TGGGame.refresh();notify('STUDIO UPGRADED')}};
