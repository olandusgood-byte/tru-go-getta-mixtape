(() => {
  ...
  function sync(){const level=Number(window.TGGGame?.getState?.()?.level)||1;let changed=false;districts.forEach(d=>{if(level>=d.level&&!state.unlocked.includes(d.id)){state.unlocked.push(d.id);changed=true}});if(changed)save();window.TGGProgression?.sync?.();return state}
