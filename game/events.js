(() => {
  ...
  function requirementsMet(event){
    return (event.requires||[]).every(r=>window.TGGInventory?.has?.(r.id,r.qty));
  }

  function requirementText(event){
    return (event.requires||[]).map(r=>{
      const item=window.TGGInventory?.catalog?.find(x=>x.id===r.id);
      return (item?.name||r.id)+' x'+r.qty;
    }).join(' + ');
  }

  function totalRuns(){
    return Object.values(state.runs||{}).reduce((sum,value)=>sum+Math.max(0,Number(value)||0),0);
  }
  ...
  function cityProfile(){
    ...
    return {
      rank,
      totalRuns:runs,
      uniqueCompleted:state.completed.length,
      streak:state.streak,
  ...
  function run(id){
    ...
    if(window.TGGDistricts?.canEnter&&!window.TGGDistricts.canEnter(event.district)){
      const level=window.TGGDistricts.get?.(event.district)?.level||1;
      window.__tggToast?.('LEVEL '+level+' REQUIRED FOR '+event.district.toUpperCase());
      return false;
    }
    if(!requirementsMet(event)){
      window.__tggToast?.('NEED INVENTORY — '+requirementText(event));
      return false;
    }
