(() => {
  ...
  function startBusinessActivity(business,mode='activity'){
    const x=business&&typeof business==='object'?business:{};
    const type=String(x.type||'BUSINESS').toUpperCase();
    const name=String(x.name||'BUSINESS');
    if(mode==='visit'){
      reward(25,5);
      toast(name.toUpperCase()+' VISITED • +$25 / +5 XP');
    }else{
      const kind=type==='MUSIC'?'record':type==='MEDIA'?'photos':type==='STYLE'?'fitness':'lobby';
      const rewards={record:[100,30],photos:[80,25],fitness:[70,24],lobby:[40,12]};
      const r=rewards[kind]||[50,15];
      reward(r[0],r[1]);
      toast(name.toUpperCase()+' ACTIVITY COMPLETE • +$'+String(r[0])+' / +'+String(r[1])+' XP');
    }
    window.TGGProgression?.sync?.();
    window.TGGCareer?.sync?.();
    window.TGGWorldSync?.sync?.();
  ...
  function setDriveAnalog(next={}){
    if(Number.isFinite(Number(next.steer)))driveAnalog.steer=Math.max(-1,Math.min(1,Number(next.steer)));
    if(Number.isFinite(Number(next.throttle)))driveAnalog.throttle=Math.max(-1,Math.min(1,Number(next.throttle)));
    if(Number.isFinite(Number(next.handbrake)))driveAnalog.handbrake=Math.max(0,Math.min(1,Number(next.handbrake)));
    return {...driveAnalog};
  }

  function setWalkAnalog(next={}){
    if(Number.isFinite(Number(next.x)))walkAnalog.x=Math.max(-1,Math.min(1,Number(next.x)));
    if(Number.isFinite(Number(next.y)))walkAnalog.y=Math.max(-1,Math.min(1,Number(next.y)));
    if(Number.isFinite(Number(next.sprint)))walkAnalog.sprint=Math.max(0,Math.min(1,Number(next.sprint)));
    return {...walkAnalog};
  }
  ...
  function updateVehiclePhysics(now){
    ...
    if(activeScreen!=='game'||!state.inVehicle){
      ...
      driveRuntime.throttle=approach(driveRuntime.throttle,0,DRIVE.throttleRelease*elapsed);
      driveRuntime.collisionFrames=0;
    ...
    const keyThrottle=(driveKeys.forward?1:0)-(driveKeys.reverse?1:0);
    const rawThrottle=Math.abs(keyThrottle)>.01?keyThrottle:driveAnalog.throttle;
    const throttleRate=Math.abs(rawThrottle)>.01?DRIVE.throttleResponse:DRIVE.throttleRelease;
    driveRuntime.throttle=approach(driveRuntime.throttle,rawThrottle,throttleRate*elapsed);
    const throttle=driveRuntime.throttle;
    const wantsForward=throttle>.05;
    const wantsReverse=throttle<-.05;
    const throttleAmount=Math.min(1,Math.abs(throttle));
    driveRuntime.handbrake=!!driveKeys.handbrake||driveAnalog.handbrake>.35;
    ...
    while(remaining>.0001){
      ...
      if(wantsForward){
        const rate=(movingReverse?DRIVE.brake:DRIVE.accel)*(.35+.65*throttleAmount);
        driveRuntime.speed=approach(driveRuntime.speed,DRIVE.maxForward*throttleAmount,rate*dt);
      }else if(wantsReverse){
        const rate=(movingForward?DRIVE.brake:DRIVE.reverseAccel)*(.35+.65*throttleAmount);
        driveRuntime.speed=approach(driveRuntime.speed,DRIVE.maxReverse*throttleAmount,rate*dt);
      }else{
      ...
      if(Math.abs(driveRuntime.speed)>.02){
        ...
        if(canMove && !canMove(nx,ny,true)){
          const slideScale=Math.max(0,Math.min(1,DRIVE.collisionSlide));
          const sx=Math.max(3,Math.min(94,state.x+Math.cos(rad)*driveRuntime.speed*dt*slideScale));
          const sy=Math.max(8,Math.min(88,state.y+Math.sin(rad)*driveRuntime.speed*dt*slideScale));
          let slid=false;
          if(canMove(sx,state.y,true)){state.x=sx;slid=true}
          if(canMove(state.x,sy,true)){state.y=sy;slid=true}
          driveRuntime.collisionFrames=slid?0:driveRuntime.collisionFrames+1;
          if(slid){
            driveRuntime.speed*=.72;
            driveRuntime.yawRate*=.62;
          }else{
            driveRuntime.speed*=Math.max(0,Math.min(.5,DRIVE.collisionDamping));
            driveRuntime.yawRate*=.28;
            if(driveRuntime.collisionFrames>=3)driveRuntime.speed=0;
          }
        }else{
          state.x=nx;
          state.y=ny;
  ...
  function updatePlayerPhysics(now){
    ...
    let rawX=Math.abs(keyX)>.01?keyX:walkAnalog.x;
    ...
    const sprinting=(!!walkKeys.sprint||walkAnalog.sprint>.35)&&inputMag>.08;
