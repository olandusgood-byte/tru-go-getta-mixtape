(() => {
  ...
  const screens=['menu','creator','game','pause','career','contentBoard'];
  function show(id){screens.forEach(s=>$(s)?.classList.toggle('active',s===id));$('hud').classList.toggle('hidden',!['game','career','contentBoard'].includes(id))}
  ...
  function reward(cash,xp){state.cash+=cash;addXp(xp);save()}
  ...
  window.__tggToast=toast;window.TGGGame={getState:()=>state,show,refresh:update,reward,save};
