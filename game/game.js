(() => {
  ...
  function show(id){screens.forEach(s=>$(s).classList.toggle('active',s===id));$('hud').classList.toggle('hidden',id!=='game')}
  function toast(t){$('toast').textContent=t;$('toast').classList.add('show');clearTimeout(window.__t);window.__t=setTimeout(()=>$('toast').classList.remove('show'),1800)}
  ...
  function update(){ $('hudName').textContent=state.name;$('hudLevel').textContent=state.level;$('hudCash').textContent=state.cash;$('hudXp').textContent=state.xp;$('hudNext').textContent=state.level*100;$('player').style.left=state.x+'%';$('player').style.top=state.y+'%';$('missionStatus').textContent=state.accepted?'Mission active — finish the job.':'Find M and start a mission.'; if(state.mission) $('missionBtn').textContent=state.accepted?'COMPLETE MISSION':'TAKE MISSION'; }
  function addXp(n){state.xp+=n;while(state.xp>=state.level*100){state.xp-=state.level*100;state.level++;toast('LEVEL UP — LEVEL '+state.level)} update()}
  ...
  function mission(){if(!state.mission){state.mission='studio-run';state.accepted=false;toast('M has a job for you.');update();return}if(!state.accepted){state.accepted=true;toast('MISSION ACCEPTED — get to the marked spot');update();return}if(Math.abs(state.x-72)<10&&Math.abs(state.y-36)<10){state.cash+=250;addXp(50);state.mission=null;state.accepted=false;toast('MISSION COMPLETE +$250 +50 XP');save();update()}else toast('Move closer to M to finish the mission')}
  $('newGame').onclick=()=>show('creator');$('continueGame').onclick=()=>{load();show('game');toast('WELCOME BACK')};$('startGame').onclick=()=>{state={...state,name:($('stageName').value.trim()||'PLAYER'),style:$('styleChoice').value,x:50,y:55,cash:0,xp:0,level:1,mission:null,accepted:false};save();show('game');toast('CITY LOADED — LET\'S GET IT')};$('missionBtn').onclick=mission;$('saveBtn').onclick=save;$('pauseBtn').onclick=()=>show('pause');$('resumeBtn').onclick=()=>show('game');$('menuBtn').onclick=()=>show('menu');
