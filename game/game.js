(() => {
  ...
  function show(id){screens.forEach(s=>$(s)?.classList.toggle('active',s===id));$('hud').classList.toggle('hidden',!['game','career','contentBoard','expansionBoard','bridge'].includes(id));update()}
  ...
  function addXp(n){state.xp+=Math.max(0,n);while(state.xp>=state.level*100){state.xp-=state.level*100;state.level++;toast('LEVEL UP — LEVEL '+state.level)}update()}
  function reward(cash,xp){state.cash+=Math.max(0,cash||0);addXp(xp||0);save()}
  function spend(cash){const n=Math.max(0,cash||0);if(state.cash<n){toast('NOT ENOUGH CASH');return false}state.cash-=n;update();save();return true}
