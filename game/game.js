(() => {
  ...
  function driveVehicle(control){
    ...
    const nextX=Math.max(3,Math.min(94,state.x+Math.cos(rad)*driveStep*direction));
    const nextY=Math.max(8,Math.min(88,state.y+Math.sin(rad)*driveStep*direction));
    const constrained=window.TGGWorld3D?.constrainPercent?.(nextX,nextY,state.x,state.y);
    const resolvedX=Number(constrained?.x??nextX);
    const resolvedY=Number(constrained?.y??nextY);
    const moved=Math.abs(resolvedX-state.x)>.001||Math.abs(resolvedY-state.y)>.001;
    if(!moved){
      toast('CAR BLOCKED — STEER BACK TO THE STREET');
      return false;
    }
    state.x=resolvedX;
    state.y=resolvedY;
