(() => {
  ...
  const steps = [
    ...
    { id:'show', name:'Hit The Street Show', description:'Take the finished project into the city.', done: () => (window.TGGExpansion?.state?.completed || []).includes('street-show') }
  ];
  ...
  function current(){ sync(); return steps.find(step=>!state.completed.includes(step.id)) || steps[steps.length-1]; }
  ...
  function start(){ state.started=true; save(); sync(); window.__tggToast?.('CREATIVE GOLDEN PATH STARTED'); }
