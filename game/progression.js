(() => {
  ...
  function sync(){
    ...
    let changed=false;
    achievements.forEach(a=>{if(!state.unlocked.includes(a.id)&&a.test(p,c,content,expansion,events,circuits)){state.unlocked.push(a.id);changed=true;window.__tggToast?.('ACHIEVEMENT UNLOCKED — '+a.name)}});
    if(changed)save();
    render();
    return state;
  }
