(() => {
  ...
  function run(){
    ...
    const expansion=window.TGGExpansion; const mapped=Array.isArray(expansion?.activities)&&expansion.activities.length>=3&&typeof expansion?.districtReady==='function'&&expansion.activities.every(a=>typeof a.district==='string'&&expansion.districtReady(a)===!!window.TGGDistricts?.canEnter?.(a.district));
    check('expansion-district-gate',mapped,'expansion activities are connected to district gating');
