(() => {
  ...
  function run(){
    ...
    ['menu','creator','game','career','contentBoard','expansionBoard','progressionBoard','inventoryBoard','bridge','pause','hud'].forEach(id=>check(id,!!document.getElementById(id),'required screen/control host exists'));
    ['missionBtn','saveBtn','careerBtn','contentBtn','advanceContentBtn','expansionBtn','progressionBtn','inventoryBtn','bridgeBtn','chainBtn'].forEach(id=>check(id,!!document.getElementById(id),'required action exists'));
    ...
    const bad=report.filter(x=>x.status==='FAIL');window.TGGQA={report,passed:bad.length===0,repair(){window.TGGSave?.repair?.();return !!document.getElementById('hud')&&!!window.TGGGame&&!!window.TGGProgression&&!!window.TGGChains&&!!window.TGGDistricts&&!!window.TGGSave&&!!window.TGGInventory}};return window.TGGQA;
  }
  window.TGGQA={run};
})();
