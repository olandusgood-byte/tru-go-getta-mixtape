(() => {
  ...
  function run(){
    ...
    const bad=report.filter(x=>x.status==='FAIL');window.TGGQA={report,passed:bad.length===0,repair(){return !!document.getElementById('hud')&&!!window.TGGGame&&!!window.TGGProgression&&!!window.TGGChains&&!!window.TGGDistricts&&!!window.TGGSave}};return window.TGGQA;
  }
  window.TGGQA={run};
})();
