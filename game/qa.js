(() => {
  ...
  function run(){
    ...
    const expansion=window.TGGExpansion; const mappedExpansion=Array.isArray(expansion?.activities)&&expansion.activities.length>=3&&typeof expansion?.districtReady==='function'&&expansion.activities.every(a=>typeof a.district==='string'&&expansion.districtReady(a)===!!window.TGGDistricts?.canEnter?.(a.district));
    check('expansion-district-gate',mappedExpansion,'expansion activities are connected to district gating');
    const missions=window.TGGContent?.content?.missions||[];
    check('content-district-gating',missions.length===3&&missions.every(m=>typeof m.district==='string'&&typeof window.TGGDistricts?.canEnter==='function'),'content missions declare districts and expose district gate API');
