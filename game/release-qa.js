(() => {
  ...
  function run(){
    ...
    test('local storage',typeof localStorage!=='undefined');const av=window.TGGAvatar?.get?.()||{};test('avatar state',typeof av.skin==='string'&&typeof av.hair==='string'&&typeof av.top==='string'&&typeof av.accent==='string');test('avatar save integration',window.TGGSave?.keys?.avatar==='tgg-avatar-v1');const ws=window.TGGWorldSync?.snapshot?.()||{};test('world sync packet',ws.schema==='tgg-world-sync-v1'&&ws.version==='1.6.0'&&!!ws.player&&!!ws.avatar&&!!ws.career);test('world sync offline default',window.TGGWorldSync?.status?.().status==='offline_ready');
