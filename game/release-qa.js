(() => {
  ...
  function run(){
    checks.length=0;
    test('game api',!!window.TGGGame);test('career api',!!window.TGGCareer);test('content api',!!window.TGGContent);test('expansion api',!!window.TGGExpansion);test('progression api',!!window.TGGProgression);test('bridge api',!!window.TGGBridge);test('chain api',!!window.TGGChains);test('district api',!!window.TGGDistricts);test('save api',!!window.TGGSave);test('inventory api',!!window.TGGInventory);test('crew api',!!window.TGGCrew);test('events api',!!window.TGGEvents);test('economy api',!!window.TGGEconomy&&typeof window.TGGEconomy.apply==='function');test('lifestyle api',!!window.TGGLifestyle&&typeof window.TGGLifestyle.buyProperty==='function'&&typeof window.TGGLifestyle.buyVehicle==='function'&&typeof window.TGGLifestyle.movementMultiplier==='function');
    ['menu','creator','game','career','contentBoard','expansionBoard','progressionBoard','inventoryBoard','crewBoard','eventsBoard','lifestyleBoard','bridge','pause','hud'].forEach(id=>test('dom:'+id,!!document.getElementById(id)));
    ['newGame','continueGame','startGame','missionBtn','careerBtn','contentBtn','advanceContentBtn','expansionBtn','progressionBtn','inventoryBtn','crewBtn','eventsBtn','lifestyleBtn','bridgeBtn','chainBtn','saveBtn','pauseBtn','resumeBtn','menuBtn','recordBtn','mixtapeBtn','upgradeBtn'].forEach(id=>test('control:'+id,!!document.getElementById(id)));
    ...
    test('lifestyle catalog',Array.isArray(window.TGGLifestyle?.properties)&&window.TGGLifestyle.properties.length>=3&&Array.isArray(window.TGGLifestyle?.vehicles)&&window.TGGLifestyle.vehicles.length>=3);test('lifestyle persistence',typeof window.TGGLifestyle?.save==='function'&&typeof window.TGGLifestyle?.load==='function');test('lifestyle movement',typeof window.TGGLifestyle?.movementMultiplier==='function'&&window.TGGLifestyle.movementMultiplier()>=1);
