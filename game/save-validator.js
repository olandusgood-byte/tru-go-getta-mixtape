(() => {
  ...
  function repair(){
    ...
    ['content','expansion','progression','chains','districts'].forEach(k=>{const v=read(keys[k],null);if(v)localStorage.setItem(keys[k],JSON.stringify(v))});
    return report();
  }
  function report(){const out={};Object.entries(keys).forEach(([name,key])=>{const v=read(key,null);out[name]=!!v&&typeof v==='object'});out.valid=Object.values(out).every(Boolean);return out}
