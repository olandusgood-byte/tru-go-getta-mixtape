...
with sync_playwright() as p:
    ...
    check('3d-collider-count',world3d.get('colliders',0)>=20,json.dumps(world3d))
    collision=page.evaluate("window.TGGWorld3D.collisionSnapshot()")
    check('mission-collision-safe',collision.get('missionReachable') is True,json.dumps(collision))
    ...
    blocked_before=page.evaluate("window.TGGGame.getState().x")
    blocked_result=page.evaluate("window.TGGGame.move(2,0)")
    ...
    check('camera-reset',abs(reset_camera.get('yaw',99))<0.001 and abs(reset_camera.get('distance',0)-10)<0.001,json.dumps(reset_camera))
