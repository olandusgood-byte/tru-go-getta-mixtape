...
with sync_playwright() as p:
    ...
    check('v123-title',page.title()=='TRU GO GETTA — Game V1.23',page.title())
    ...
    landmark_status=page.evaluate("window.TGGWorld3D.status()")
    check('3d-landmark-count',landmark_status.get('landmarks',0)>=5,json.dumps(landmark_status))
    page.evaluate("""() => {
    ...
    studio_near=page.evaluate("window.TGGWorld3D.nearestLandmark()")
    check('studio-landmark-proximity',studio_near is not None and studio_near.get('id')=='studio-row',json.dumps(studio_near))
    check('hub-prompt-visible',page.locator('#world3dPrompt').get_attribute('class').find('show')>=0,page.locator('#world3dPrompt').inner_text())
    page.keyboard.press('e')
    check('enter-studio-via-e','active' in (page.locator('#studio').get_attribute('class') or ''))
    page.click('#studioBack')
    check('return-from-studio','active' in (page.locator('#game').get_attribute('class') or ''))
