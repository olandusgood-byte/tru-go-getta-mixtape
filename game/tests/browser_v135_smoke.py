...
try:
  ...
  with sync_playwright() as p:
    ...
    check('title',page.title()=='TRU GO GETTA — Game V1.35 3D',page.title())
    ...
    check('street-audience-api',page.evaluate("!!window.TGGStreetAudience && window.TGGStreetAudience.CREWS.length===3"))
    audience=page.evaluate("window.TGGStreetAudience.audienceProfile()")
    check('audience-memory-built',audience.get('uniqueSeen',0)>=4 and audience.get('totalAppearances',0)>=12,json.dumps(audience))
    check('returning-regulars',audience.get('regulars',0)>=3,json.dumps(audience))
    block_crew=page.evaluate("window.TGGStreetAudience.crewSnapshot('block-crew')")
    check('block-crew-locked',block_crew.get('bond',0)>=6 and block_crew.get('tier') in ['LOCKED IN','CORE CREW'],json.dumps(block_crew))
    ranked=page.evaluate("""() => window.TGGStreetAudience.rankCandidates(
      [{index:5,distance:1},{index:0,distance:8},{index:1,distance:9}],
      'downtown-cypher'
    ).map(x=>x.index)""")
    check('audience-aware-crowd-selection',ranked[0] in [0,1],json.dumps(ranked))
    audience_summary=page.locator('#streetSetHud [data-audience-summary]').inner_text()
    check('audience-summary-visible','AUDIENCE' in audience_summary and 'CREW' in audience_summary,audience_summary)
    stored_audience=page.evaluate("JSON.parse(localStorage.getItem('tgg-street-events-v1')||'{}').audience")
    check('audience-persistence',isinstance(stored_audience,dict) and len(stored_audience.get('people',{}))>=4,json.dumps(stored_audience))
    check('audience-single-store',page.evaluate("localStorage.getItem('tgg-street-audience-v1')") is None)
    audience_unlocks=page.evaluate("window.TGGProgression.sync().unlocked")
    check('audience-regulars-achievement','audience-regulars' in audience_unlocks,json.dumps(audience_unlocks))
    check('street-crew-achievement','street-crew-locked' in audience_unlocks,json.dumps(audience_unlocks))
    after_audience=page.evaluate("({cash:window.TGGGame.getState().cash,xp:window.TGGGame.getState().xp})")
    check('audience-no-reward',after_audience==before_event,json.dumps([before_event,after_audience]))
