...
wait().then(api=>{
 ...
 const npcMarkers=[];for(let i=0;i<28;i++){const n=new THREE.Mesh(new THREE.CapsuleGeometry?.(.28,.9,4,8)||new THREE.BoxGeometry(.5,1.6,.5),mk(0x6d7784,.86,.05));n.position.set(-140+(i%14)*20,.8,120+Math.floor(i/14)*20);n.userData.tggNpcSchedule={shift:(i%4),home:i%7,work:(i+3)%9};root.add(n);npcMarkers.push(n)}
