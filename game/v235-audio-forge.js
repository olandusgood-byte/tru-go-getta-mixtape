(() => {
  ...
  function applyProfile(id='street'){
    ...
    if(hasDOM()){try{localStorage.setItem('tgg-v235-audio',state.profile)}catch{}renderUI()}return status();
  }
  ...
  function ensureUI(){
    if(!hasDOM())return;document.body.classList.add('tgg-v235');const badge=document.querySelector('.v201-badge');if(badge)badge.textContent=VERSION;
    const top=document.querySelector('.topbar');if(top&&!document.getElementById('v235ForgeBtn')){const b=document.createElement('button');b.id='v235ForgeBtn';b.className='v235-forge-btn';b.type='button';b.textContent='AUDIO';b.addEventListener('click',()=>panel?.classList.toggle('active'));top.appendChild(b)}
    if(!panel){panel=document.createElement('aside');panel.id='v235ForgePanel';panel.className='v235-forge-panel';panel.innerHTML='<div><small>TGG NATIVE AUDIO</small><b>AUDIO + SPATIAL FORGE</b></div><div class="v235-profiles"><button data-v235="street">STREET</button><button data-v235="club">CLUB</button><button data-v235="studio">STUDIO</button><button data-v235="cinematic">CINEMATIC</button></div><div class="v235-actions"><button id="v235Unlock">ENABLE AUDIO</button><button id="v235Toggle">AUDIO: ON</button><button id="v235Test">TEST SOUND</button><button id="v235Restore">RESTORE</button></div><div id="v235Stats"></div>';document.body.appendChild(panel);panel.querySelectorAll('[data-v235]').forEach(b=>b.onclick=()=>applyProfile(b.dataset.v235));document.getElementById('v235Unlock').onclick=unlock;document.getElementById('v235Toggle').onclick=()=>setEnabled(!state.enabled);document.getElementById('v235Test').onclick=()=>play('confirm');document.getElementById('v235Restore').onclick=restore}
    const city=document.querySelector('.city');if(city&&!hud){hud=document.createElement('div');hud.id='v235ForgeHud';hud.className='v235-forge-hud';city.appendChild(hud)}bridge();renderUI();
  }
  function renderUI(){if(!hasDOM())return;if(hud)hud.innerHTML='<small>TGG AUDIO FORGE</small><b>'+state.profile.toUpperCase()+'</b><span>'+(state.unlocked?'AUDIO READY':'CLICK ENABLE AUDIO')+'</span>';const s=document.getElementById('v235Stats');if(s)s.textContent=state.events+' EVENTS • '+(state.enabled?'ON':'OFF');panel?.querySelectorAll('[data-v235]').forEach(b=>b.classList.toggle('active',b.dataset.v235===state.profile))}
  function load(){if(!hasDOM())return;try{const p=localStorage.getItem('tgg-v235-audio');if(core()?.profiles?.includes(p))state.profile=p}catch{}}
  ...
  if(hasDOM()){window.TGGV235=api;['pointerdown','keydown','touchstart'].forEach(ev=>window.addEventListener(ev,()=>unlock(),{once:true,passive:true}));ensureUI();requestAnimationFrame(tick)}
})();