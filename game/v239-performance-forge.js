(() => {
  ...
  function captureBaseline(){
    ...
    state.baseline={pixelRatio:r.getPixelRatio?.()||window.devicePixelRatio||1,shadowEnabled:!!r.shadowMap?.enabled,crowdDensity:window.TGGV236?.status?.()?.density??1,effectsEnabled:window.TGGV238?.status?.()?.enabled!==false,lightingQuality:window.TGGV231?.status?.()?.quality||'auto'};
  ...
  function applyTier(id='balanced'){
    ...
    const crowd=window.TGGV236?.status?.();if(crowd&&Math.abs((crowd.density||1)-cfg.crowd)>.04)window.TGGV236?.setDensity?.(cfg.crowd);
  ...
  function restore(){
    ...
    if(b){window.TGGV236?.setDensity?.(b.crowdDensity);window.TGGV238?.setEnabled?.(b.effectsEnabled);window.TGGV231?.setQuality?.(b.lightingQuality)}
