(() => {
  const VERSION='V2.39 TGG STAGE + PERFORMANCE FORGE 100';
  ...
  function buildStage(){
    ...
    const cfg=core()?.show?.(state.show)||{},g=new THREE.Group();g.name='tgg-stage-performance';g.userData.v239=true;g.position.set(0,0,31);
  ...
  function ensureUI(){
    ...
    const top=document.querySelector('.topbar');if(top&&!document.getElementById('v239ForgeBtn')){const b=document.createElement('button');b.id='v239ForgeBtn';b.className='v239-forge-btn';b.textContent='STAGE';b.onclick=()=>panel?.classList.toggle('active');top.appendChild(b)}
    if(!panel){panel=document.createElement('aside');panel.id='v239ForgePanel';panel.className='v239-forge-panel';panel.innerHTML='<small>TGG NATIVE 3D</small><b>STAGE + PERFORMANCE FORGE</b><div><button data-show="concert">CONCERT</button><button data-show="battle">RAP BATTLE</button><button data-show="club">CLUB</button><button data-show="showcase">SHOWCASE</button></div><button id="v239Stop">STOP SHOW</button><button id="v239Toggle">STAGE: ON</button><span id="v239Stats"></span>';document.body.appendChild(panel);panel.querySelectorAll('[data-show]').forEach(b=>b.onclick=()=>start(b.dataset.show));document.getElementById('v239Stop').onclick=stop;document.getElementById('v239Toggle').onclick=()=>setEnabled(!state.enabled)}
    const city=document.querySelector('.city');if(city&&!hud){hud=document.createElement('div');hud.id='v239ForgeHud';hud.className='v239-forge-hud';city.appendChild(hud)}bridgeEvents();renderUI();
  }
  function renderUI(){if(!hasDOM())return;const q=id=>document.getElementById(id);if(q('v239Toggle'))q('v239Toggle').textContent='STAGE: '+(state.enabled?'ON':'OFF');if(q('v239Stats'))q('v239Stats').textContent=(state.active?state.show.toUpperCase():'IDLE')+' • '+quality().toUpperCase();if(hud)hud.textContent='STAGE • '+(state.active?state.show.toUpperCase():'READY');panel?.querySelectorAll('[data-show]').forEach(b=>b.classList.toggle('active',state.active&&b.dataset.show===state.show))}
  ...
  const api={version:VERSION,layers:LAYERS,status,start,stop,rebuild,setEnabled};globalThis.TGGV239=api;if(hasDOM()){window.TGGV239=api;ensureUI();requestAnimationFrame(tick)}
})();