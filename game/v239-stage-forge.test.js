(() => {
  const forge=globalThis.__V239ForgeUnderTest,assert=(ok,msg)=>{if(!ok)throw new Error(msg)};
  assert(forge,'V2.39 Stage Forge adapter API must exist');
  ...
  assert(s.version==='V2.39 TGG STAGE + PERFORMANCE FORGE 100','version');
