(()=>{
  ...
  function load(){
    try{
      ...
      return {...defaults,...saved,properties:{...defaults.properties,...(saved.properties||{})},chainProgress:{...(saved.chainProgress||{})},completedChains:Array.isArray(saved.completedChains)?saved.completedChains:[]};
  ...
  function getStatus(){
    return {version:VERSION,...state,reputationTier:reputationTier(),relationScore:relationScore(),propertyBonus:propertyBonus(),activeChain:activeChain(),features:[
      ...
      'relationship-gated-moves','daily-world-progression','property-bonus-loop','career-score-system'
    ]};
  }
  function boot(){
    ensurePanel();render();hookOpportunity();
    document.documentElement.dataset.tggV400='on';
    window.TGGWorldSystems={version:VERSION,getStatus,buyOrUpgrade,applyConsequence,coolConsequence,onOpportunityComplete,eligibleMove,reputationTier,propertyBonus};
