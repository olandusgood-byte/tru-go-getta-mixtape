(()=>{
  'use strict';
  const VERSION='V5.75';
  const LAYERS=['V5.67','V5.68','V5.69','V5.70','V5.71','V5.72','V5.73','V5.74','V5.75'];
  ...
  wait().then(()=>{
    try{
      ...
      const root=new THREE.Group();root.name='TGG_V567_V575_VISUAL_MEGA';scene.add(root);
      ...
      const vehicleDetail=new THREE.Group();vehicleDetail.name='TGG_V571_VEHICLE_SIGNAL';car.add(vehicleDetail);
      ...
      window.TGGV575VisualMega={version:VERSION,getStatus:status,root};
      document.documentElement.dataset.tggVisualMega='v575';
      window.dispatchEvent(new CustomEvent('tgg:v575-visual-mega-ready',{detail:status()}));
    }catch(error){window.TGGV575VisualMega={version:VERSION,getStatus:()=>({version:VERSION,ok:false,error:String(error?.message||error)})};document.documentElement.dataset.tggVisualMega='fallback'}
  });
})();