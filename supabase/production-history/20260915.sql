...
-- ============================================================
-- MIGRATION 20260915064042 index_browser_qa_results_run_id
-- created_by: trugmusicgroup@gmail.com
-- statement_count: 1

create index if not exists tgg_browser_qa_results_run_id_idx
on public.tgg_browser_qa_results(run_id);

-- ============================================================
-- MIGRATION 20260915064119 index_active_qa_audit_build_playtest_fks
-- created_by: trugmusicgroup@gmail.com
-- statement_count: 1

create index if not exists tgg_visual_qa_results_design_spec_id_idx
  on public.tgg_visual_qa_results(design_spec_id);
create index if not exists tgg_visual_qa_results_run_id_idx
  on public.tgg_visual_qa_results(run_id);
create index if not exists tgg_veil_audit_ledger_actor_user_id_idx
  on private.tgg_veil_audit_ledger(actor_user_id);
create index if not exists tgg_build_failures_run_id_idx
  on public.tgg_build_failures(run_id);
create index if not exists tgg_playtest_runs_build_run_id_idx
  on public.tgg_playtest_runs(build_run_id);
create index if not exists tgg_playtest_runs_scenario_id_idx
  on public.tgg_playtest_runs(scenario_id);

-- ============================================================
-- MIGRATION 20260915064251 index_active_production_readiness_fks
-- created_by: trugmusicgroup@gmail.com
-- statement_count: 1

create index if not exists tgg_production_smoke_results_test_id_idx
  on public.tgg_production_smoke_results(test_id);
create index if not exists tgg_launch_readiness_latest_qa_run_id_idx
  on public.tgg_launch_readiness(latest_qa_run_id);
create index if not exists tgg_build_runs_release_id_idx
  on public.tgg_build_runs(release_id);
create index if not exists tgg_environment_drift_results_config_id_idx
  on public.tgg_environment_drift_results(config_id);
create index if not exists tgg_release_gate_evaluations_score_run_id_idx
  on public.tgg_release_gate_evaluations(score_run_id);

-- ============================================================
-- MIGRATION 20260915064716 harden_protected_audio_browser_evidence_service_bridge
-- created_by: trugmusicgroup@gmail.com
-- statement_count: 1
...
as $function$
begin
  if p_flow_key='protected_audio_runtime' then
    raise exception 'PROTECTED_AUDIO_EDGE_QA_REQUIRED' using errcode='42501';
  end if;
  return private.tgg_runtime_browser_observe_v1_impl(p_flow_key,p_capture);
end
$function$;

create or replace function public.tgg_server_protected_audio_browser_observe_v2(
  p_user_id uuid,
  p_client_capture jsonb default '{}'::jsonb
)
returns jsonb
language plpgsql
security definer
...
declare
  v_role text := coalesce(current_setting('request.jwt.claim.role',true),'');
  v_claims text := coalesce(current_setting('request.jwt.claims',true),'');
  v_load_ms integer := greatest(1,least(120000,coalesce((p_client_capture->>'load_ms')::integer,1)));
  v_page_path text := coalesce(p_client_capture->>'page_path','');
  v_capture jsonb;
begin
  if v_role <> 'service_role'
     and v_claims not like '%"role":"service_role"%' then
    raise exception 'SERVICE_ROLE_REQUIRED' using errcode='42501';
  end if;

  if p_user_id is null or not exists(select 1 from auth.users u where u.id=p_user_id) then
    raise exception 'QA_USER_INVALID' using errcode='22023';
  end if;

  if v_page_path !~ '^/functions/v1/tgg-audio-access\\?qa=protected_audio(?:&.*)?$' then
    raise exception 'QA_PAGE_PATH_INVALID' using errcode='22023';
  end if;

  if coalesce((p_client_capture->>'playback_started')::boolean,false) is not true then
    raise exception 'PLAYBACK_NOT_CONFIRMED' using errcode='22023';
  end if;

  v_capture := jsonb_build_object(
    'build','CREATOR-OS-AUTO-QA-V1',
  ...
  perform set_config(
    'request.headers',
    jsonb_build_object('origin','https://xsofowzvwetamhyuvlpj.supabase.co')::text,
    true
  );

  return private.tgg_runtime_browser_observe_v1_impl('protected_audio_runtime',v_capture);
end
$function$;

revoke all on function public.tgg_server_protected_audio_browser_observe_v2(uuid,jsonb) from public,anon,authenticated;
grant execute on function public.tgg_server_protected_audio_browser_observe_v2(uuid,jsonb) to service_role;
...
-- ============================================================
-- MIGRATION 20260915065131 fail_close_launch_readiness_on_browser_evidence
-- created_by: trugmusicgroup@gmail.com
-- statement_count: 1
...
begin
  ...
  if v_def is null then
    raise exception 'LAUNCH_READINESS_INTERNAL_NOT_FOUND';
  end if;

  if position('browser_evidence as (' in v_def) > 0 then
    return;
  end if;

  v_def := replace(
    v_def,
    $old$call_health as (select public.tgg_call_validation_final_health() j),
external as (select public.tgg_external_completion_state() j),$old$,
    $new$call_health as (select public.tgg_call_validation_final_health() j),
browser_evidence as (
  select
    count(*) filter(where evidence_required) as required,
    count(*) filter(where evidence_required and browser_verified and status='passed') as verified
  from public.tgg_browser_runtime_evidence_gate
),
external as (select public.tgg_external_completion_state() j),$new$
  );

  v_def := replace(
    v_def,
    $old$    coalesce(((select j->>'fully_connected' from external))::boolean,false) fully_connected
),$old$,
    $new$    coalesce((select required>0 and verified=required from browser_evidence),false) browser_evidence_ok,
    coalesce(((select j->>'fully_connected' from external))::boolean,false) fully_connected
),$new$
  );
  ...
  v_def := replace(
    ...
    $old$      drift_ok
      and features_ok
      and payment_link_ok$old$,
    $new$      drift_ok
      and features_ok
      and browser_evidence_ok
      and payment_link_ok$new$
  );

  v_def := replace(v_def,'''version'',''LAUNCH-READINESS-ONE-FINAL-1.4''','''version'',''LAUNCH-READINESS-ONE-FINAL-1.5''');

  v_def := replace(
    v_def,
    $old$    'drift_guard',drift_ok,
    'edge_function_capacity',$old$,
    $new$    'drift_guard',drift_ok,
    'browser_evidence',jsonb_build_object(
      'required',(select required from browser_evidence),
      'verified',(select verified from browser_evidence),
      'remaining',greatest((select required-verified from browser_evidence),0),
      'ready',browser_evidence_ok,
      'required_for_core',true
    ),
    'edge_function_capacity',$new$
  );

  if position('browser_evidence_ok' in v_def)=0
     or position('LAUNCH-READINESS-ONE-FINAL-1.5' in v_def)=0 then
    raise exception 'LAUNCH_READINESS_PATCH_DID_NOT_APPLY';
  end if;
