...
with sync_playwright() as p:
    ...
    check('v115-title',page.title()=='TRU GO GETTA — Game V1.15',page.title())
    ...
    run_results=page.evaluate("[window.TGGEvents.run('street-cypher'),window.TGGEvents.run('street-cypher'),window.TGGEvents.run('street-cypher')]")
    ...
    check('city-variety-streak',studio_run is True and profile2.get('streak')==2 and profile2.get('bestStreak')>=2,json.dumps(profile2))
