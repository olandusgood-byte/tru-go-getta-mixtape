...
app.get('/enroll', (_req, res) => res.type('html').send(`<!doctype html><html><head><meta charset="utf-8"><title>TGG Browser Worker Enrollment</title><style>body{font-family:Arial;background:#080808;color:#fff;max-width:720px;margin:50px auto;padding:24px}button{background:#e50914;color:#fff;border:0;padding:12px 18px;border-radius:8px;font-weight:700}input{display:block;width:100%;margin:8px 0;padding:12px;background:#151515;color:#fff;border:1px solid #333;border-radius:8px;box-sizing:border-box}pre{white-space:pre-wrap;background:#111;padding:15px;border-radius:8px}</style></head><body><h1>TGG Self-Hosted Browser Worker</h1><p>Owner-only enrollment. Your authenticated session is handed directly to this worker; worker credentials never leave the server.</p><input id="email" type="email" placeholder="Owner email"><input id="password" type="password" placeholder="Owner password"><button id="go">Enroll Worker</button><pre id="out">Waiting…</pre><script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script><script>(async()=>{const S=window.supabase.createClient(${JSON.stringify(SUPABASE_URL)},${JSON.stringify(SUPABASE_KEY)});document.querySelector('#go').onclick=async()=>{const out=document.querySelector('#out');try{const email=document.querySelector('#email').value.trim(),password=document.querySelector('#password').value;const a=await S.auth.signInWithPassword({email,password});if(a.error)throw a.error;const sess=a.data?.session;if(!sess)throw new Error('No authenticated session returned.');const b=await fetch('/enroll/session',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({access_token:sess.access_token,refresh_token:sess.refresh_token})});const bj=await b.json();if(!b.ok)throw new Error(bj.error||'Enrollment failed');out.textContent='Worker enrolled and browser session bootstrapped. Refresh session persisted. TGG certification started.'}catch(e){out.textContent='ERROR: '+(e.message||String(e))}}})();</script></body></html>`));
app.post('/enroll/session', async (req, res) => { try { const access=String(req.body?.access_token||'').trim(); const refresh=String(req.body?.refresh_token||'').trim(); if(!access||!refresh)return res.status(400).json({error:'owner_session_required'}); if(!workerId||!workerToken)return res.status(503).json({error:'worker_not_configured'}); const owner=await verifyOwnerSession(access); if(!owner.ok)return res.status(403).json({error:'owner_auth_failed'}); const worker=await verifyWorkerCredential(workerId,workerToken); if(!worker.ok)return res.status(403).json({error:'worker_auth_failed'}); ownerSession={access_token:access,refresh_token:refresh}; await persistOwnerRefreshToken(workerId,workerToken,refresh); last={status:'bootstrapped',worker_id:workerId,updated_at:new Date().toISOString()}; scheduleBootstrapLoop(loop); return res.json({ok:true,worker_id:workerId,version:RUNTIME_VERSION,certification_started:true,session_persisted:true}); } catch(error) { console.error(JSON.stringify({tgg_owner_enroll:true,ok:false,error:error?.message||String(error)})); return res.status(500).json({error:'owner_enrollment_failed'}); } });
