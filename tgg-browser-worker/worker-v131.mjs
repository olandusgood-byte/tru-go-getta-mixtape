...
async function refreshWorkerToken(){const secret=String(process.env.TGG_WORKER_BOOTSTRAP_SECRET||'');if(!workerId||!secret)return false;const res=await rpc.rpc('tgg_browser_cert_worker_bootstrap',{p_worker_id:workerId,p_bootstrap_secret:secret});if(res.error||!res.data?.worker_token)return false;workerToken=res.data.worker_token;console.log(JSON.stringify({tgg_worker_token_refreshed:true,worker_id_configured:true}));return true;} async function loop(){console.log(JSON.stringify({tgg_loop_enter:true,running,worker_id_configured:Boolean(workerId),worker_token_configured:Boolean(workerToken)}));if(running||!workerId||!workerToken)return;running=true;try{console.log(JSON.stringify({tgg_heartbeat_start:true}));let hb=await rpc.rpc('tgg_browser_cert_worker_heartbeat',{p_worker_id:workerId,p_token:workerToken,p_metadata:{host:'render',version:RUNTIME_VERSION,session_bootstrapped:Boolean(ownerSession?.access_token),multi_flow_browser_runtime:true}});if(hb.error?.message==='WORKER_AUTH_FAILED'&&await refreshWorkerToken()) hb=await rpc.rpc('tgg_browser_cert_worker_heartbeat',{p_worker_id:workerId,p_token:workerToken,p_metadata:{host:'render',version:RUNTIME_VERSION,session_bootstrapped:Boolean(ownerSession?.access_token),multi_flow_browser_runtime:true}});if(hb.error){console.error(JSON.stringify({tgg_heartbeat_error:true,error:hb.error.message||String(hb.error)}));throw hb.error;}console.log(JSON.stringify({tgg_heartbeat_ok:true}));if(!ownerSession?.access_token){const restored=await restoreOwnerSessionFromRefreshToken();if(!restored){last={status:'awaiting_owner_session',updated_at:new Date().toISOString()};return;}}console.log(JSON.stringify({tgg_claim_start:true,session_present:Boolean(ownerSession?.access_token)}));const claim=await rpc.rpc('tgg_browser_cert_worker_claim',{p_worker_id:workerId,p_token:workerToken,p_lease_seconds:300});if(claim.error)throw claim.error;const job=extractClaimJob(claim.data);if(!job?.id){last={status:'idle',updated_at:new Date().toISOString()};return;}last={status:'running',flow_key:job.flow_key,job_id:job.id,updated_at:new Date().toISOString()};try{const result=job.flow_key==='protected_audio_runtime'?await runProtectedAudio(job):await runGeneric(job);await complete(job,'passed',result,result.evidence||result.captures||[]);last={status:'passed',flow_key:job.flow_key,job_id:job.id,updated_at:new Date().toISOString()};}catch(e){const failure={error:e.message||String(e),real_browser_attempt:true,at:new Date().toISOString()};await complete(job,'failed',failure,[{type:'browser_failure',sha256:sha256(JSON.stringify(failure)),artifact_uri:`tgg://browser-cert/${job.id}/failure.json`,metadata:failure}]);last={status:'failed',flow_key:job.flow_key,job_id:job.id,error:failure.error,updated_at:new Date().toISOString()};}}catch(e){last={status:'worker_error',error:e.message||String(e),updated_at:new Date().toISOString()};}finally{running=false;}}
