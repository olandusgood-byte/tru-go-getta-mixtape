...
import { tggCoreEnabled, tggWorkerHeartbeat, tggWorkerClaim, tggWorkerComplete, tggStoreOwnerRefreshToken, tggRestoreOwnerRefreshToken, tggWorkerBootstrap, tggWorkerRegister, tggWorkerRecoverCertification, tggWorkerCertificationStatus, tggViewerEvent } from './tgg-core-client.mjs';
...
async function viewerEmit(browserSessionId,eventType,payload={}) {
  try { return await tggViewerEvent(browserSessionId,eventType,payload); }
  catch (e) { console.error(JSON.stringify({tgg_live_viewer:true,ok:false,event_type:eventType,error:e?.message||String(e)})); return null; }
}
let running = false;
...
async function runProtectedAudio(job){await viewerEmit(job.browser_session_id||job.payload?.browser_session_id,'flow_started',{flow_key:job.flow_key,job_id:job.id});const browser=await chromium.launch({headless:true,args:['--no-sandbox','--disable-dev-shm-usage','--autoplay-policy=no-user-gesture-required']});const context=await browser.newContext({viewport:{width:1440,height:1000}});const page=await context.newPage();const captures=[];const capture=async(type,payload)=>{const body=Buffer.from(JSON.stringify({type,at:new Date().toISOString(),...payload},null,2));captures.push({type,sha256:sha256(body),artifact_uri:`tgg://browser-cert/${job.id}/${type}.json`,metadata:payload});};const started=Date.now();try{const coreUrl=String(process.env.TGG_CORE_URL||'').replace(/\/$/,''); const url=job.url||`${coreUrl}/v1/storage/object-auth/audio/${encodeURIComponent(String(job.object_key||job.storage_key||'protected-audio'))}`;const navigation=await navigateProtectedAudioQa(page,url,job);await viewerEmit(job.browser_session_id||job.payload?.browser_session_id,'page_loaded',{url:page.url(),status:navigation.status});await capture('page_rendered',{url:page.url(),title:await page.title(),http_status:navigation.status,qa_marker:navigation.marker,content_type:navigation.contentType,trusted_navigation:navigation.trusted_navigation});const textBefore=await page.locator('body').innerText().catch(()=> '');await capture('authenticated',{session_bootstrapped:true,body_excerpt:textBefore.slice(-1200)});const qaRun=await page.evaluate(protectedAudioBrowserFlow,{supabaseUrl:process.env.TGG_CORE_URL||SUPABASE_URL,apiKey:'',accessToken:job.tgg_core_access_token||ownerSession?.access_token});if(!qaRun?.ok)throw new Error(`PROTECTED_AUDIO_QA_PAGE_FAILED:${String(qaRun?.statusText||qaRun?.error||'Unknown browser QA failure').slice(0,1000)}`);const bodyText=await page.locator('body').innerText();const playbackStarted=qaRun.playbackStarted===true;const screenshotBuffer=await page.screenshot({fullPage:true,type:'png'});const html=await page.content();const result=buildProtectedAudioCertificateResult({job,screenshotBuffer,html,bodyText,playbackStarted,elapsedMs:Date.now()-started});captures.push({type:'screenshot',sha256:result.screenshot_sha256,artifact_uri:`tgg://browser-cert/${job.id}/screenshot.png`,metadata:{url:page.url(),bytes:screenshotBuffer.length,server_pass_observed:true}});captures.push({type:'page',sha256:result.html_sha256,artifact_uri:`tgg://browser-cert/${job.id}/page.html`,metadata:{url:page.url(),bytes:Buffer.byteLength(html),title:await page.title(),server_pass_observed:true}});await capture('protected_audio_result',{playback_started:playbackStarted,pass_text_observed:true,body_excerpt:bodyText.slice(-4000),track_id:qaRun.trackId||null});await capture('browser_certificate_observation',{authenticated:true,rendered:true,playback_started:playbackStarted,pass_text_observed:true,elapsed_ms:result.elapsed_ms,challenge_echo:result.challenge_echo});await viewerEmit(job.browser_session_id||job.payload?.browser_session_id,'flow_passed',{flow_key:job.flow_key,evidence_count:captures.length});return{...result,authenticated:true,evidence_count:captures.length,captures};}finally{await context.close();await browser.close();}}
async function runRecordingStudio(job){
  const browser=await chromium.launch({headless:true,args:['--no-sandbox','--disable-dev-shm-usage','--use-fake-device-for-media-stream','--use-fake-ui-for-media-stream','--autoplay-policy=no-user-gesture-required']});
  const context=await browser.newContext({viewport:{width:1440,height:1000}}); const page=await context.newPage(); const started=Date.now();
  const consoleErrors=[]; const pageErrors=[]; page.on('console',m=>{if(m.type()==='error')consoleErrors.push(m.text().slice(0,1000));}); page.on('pageerror',e=>pageErrors.push(String(e?.message||e).slice(0,1000)));
  try{
    const accessToken=job.tgg_core_access_token||job.payload?.tgg_core_access_token||ownerSession?.access_token||''; if(!accessToken)throw new Error('RECORDING_STUDIO_AUTH_MISSING');
    const target=job.url||'https://trugogettamixtapes.blogspot.com/p/recording-studio.html';
    const response=await page.goto(target,{waitUntil:'domcontentloaded',timeout:60000}); await page.waitForLoadState('networkidle',{timeout:15000}).catch(()=>{});
    const browserResult=await page.evaluate(async()=>{
      ...
      try{
        record('secure-context',window.isSecureContext===true); record('getUserMedia-api',typeof navigator.mediaDevices?.getUserMedia==='function'); record('media-recorder-api',typeof MediaRecorder==='function'); record('audio-context-api',typeof AudioContext==='function'||typeof webkitAudioContext==='function'); record('webrtc-api',typeof RTCPeerConnection==='function');
        if(!navigator.mediaDevices?.getUserMedia)throw new Error('GET_USER_MEDIA_UNAVAILABLE');
        const stream=await navigator.mediaDevices.getUserMedia({audio:true,video:true});
        record('mic-track-live',stream.getAudioTracks().some(t=>t.readyState==='live'),stream.getAudioTracks().map(t=>t.label).join('|')); record('camera-track-live',stream.getVideoTracks().some(t=>t.readyState==='live'),stream.getVideoTracks().map(t=>t.label).join('|'));
        const recorder=new MediaRecorder(stream); const chunks=[]; recorder.ondataavailable=e=>{if(e.data?.size)chunks.push(e.data.size)}; recorder.start(100); await new Promise(r=>setTimeout(r,350));
        await new Promise((resolve,reject)=>{recorder.onstop=resolve; recorder.onerror=e=>reject(e.error||new Error('MEDIA_RECORDER_ERROR')); recorder.stop()});
        const bytes=chunks.reduce((a,b)=>a+b,0); record('media-recorder-captured',bytes>0,'bytes='+bytes);
        let rtcConnected=false; if(typeof RTCPeerConnection==='function'){const a=new RTCPeerConnection(),b=new RTCPeerConnection(); a.onicecandidate=e=>{if(e.candidate)b.addIceCandidate(e.candidate).catch(()=>{})}; b.onicecandidate=e=>{if(e.candidate)a.addIceCandidate(e.candidate).catch(()=>{})}; stream.getTracks().forEach(t=>a.addTrack(t,stream)); b.ontrack=()=>{rtcConnected=true}; const offer=await a.createOffer(); await a.setLocalDescription(offer); await b.setRemoteDescription(offer); const answer=await b.createAnswer(); await b.setLocalDescription(answer); await a.setRemoteDescription(answer); await new Promise(r=>setTimeout(r,500)); a.close(); b.close()}
        record('webrtc-loopback',rtcConnected); stream.getTracks().forEach(t=>t.stop()); record('tracks-stopped-cleanly',stream.getTracks().every(t=>t.readyState==='ended'));
      }catch(error){record('recording-exception',false,error?.message||String(error))} return {passed:checks.length>0&&checks.every(x=>x.pass),checks};
    });
    if(!browserResult.passed)throw new Error('RECORDING_STUDIO_MEDIA_CERT_FAILED:'+JSON.stringify(browserResult.checks).slice(0,6000));
    const screenshot=await page.screenshot({fullPage:true,type:'png'}); const html=await page.content(); const capture={build:'TGG-RECORDING-STUDIO-MEDIA-CERT-V1',flow_key:'recording_studio_runtime',url:page.url(),http_status:response?.status()||0,synthetic_media_devices:true,media_checks:browserResult.checks,console_errors:consoleErrors,page_errors:pageErrors,elapsed_ms:Date.now()-started};
    const evidence=[{type:'screenshot',sha256:sha256(screenshot),artifact_uri:'tgg://browser-cert/'+job.id+'/screenshot.png',metadata:{flow_key:'recording_studio_runtime',url:page.url(),synthetic_media_devices:true}},{type:'page',sha256:sha256(html),artifact_uri:'tgg://browser-cert/'+job.id+'/page.html',metadata:{flow_key:'recording_studio_runtime',url:page.url(),bytes:Buffer.byteLength(html)}},{type:'media_capture_execution',sha256:sha256(JSON.stringify(capture)),artifact_uri:'tgg://browser-cert/'+job.id+'/recording-media.json',metadata:capture}];
    return {...capture,authenticated:Boolean(accessToken),browser_execution:true,executed:true,evidence,screenshot_sha256:sha256(screenshot),html_sha256:sha256(html),elapsed_ms:Date.now()-started};
  }finally{await context.close();await browser.close()}
}
async function runGeneric(job){await viewerEmit(job.browser_session_id||job.payload?.browser_session_id,'flow_started',{flow_key:job.flow_key,job_id:job.id});const browser=await chromium.launch({headless:true,args:['--no-sandbox','--disable-dev-shm-usage']});const context=await browser.newContext({viewport:{width:1440,height:1000}});try{const coreAccessToken=job?.tgg_core_access_token||job?.payload?.tgg_core_access_token||ownerSession?.access_token;if(!coreAccessToken)throw new Error('CORE_JOB_SESSION_MISSING');if(job?.flow_key==='protected_audio_runtime'&&!ownerSession?.access_token)await restoreBrowserOwnerSession(context);const page=await context.newPage();const result=await runMultiFlowBrowser(page,{flowKey:job.flow_key,supabaseUrl:SUPABASE_URL,supabaseKey:SUPABASE_KEY,accessToken:coreAccessToken,job});await viewerEmit(job.browser_session_id||job.payload?.browser_session_id,'flow_finished',{flow_key:job.flow_key});return result;}finally{await context.close();await browser.close();}}
...
async function refreshWorkerToken(){const secret=String(process.env.TGG_WORKER_BOOTSTRAP_SECRET||'');if(!workerId||!secret)return false;const res=await rpc.rpc('tgg_browser_cert_worker_bootstrap',{p_worker_id:workerId,p_bootstrap_secret:secret});if(res.error||!res.data?.worker_token){console.error(JSON.stringify({tgg_worker_token_refresh_failed:true,error:res.error?.message||'bootstrap_response_invalid'}));return false;}workerToken=res.data.worker_token;console.log(JSON.stringify({tgg_worker_token_refreshed:true,worker_id_configured:true}));return true;} async function loop(){
  ...
  running=true;try{console.log(JSON.stringify({tgg_heartbeat_start:true}));let hb=null;try{hb=await tggWorkerHeartbeat({host:'render',version:RUNTIME_VERSION,session_bootstrapped:Boolean(ownerSession?.access_token),multi_flow_browser_runtime:true});}catch(e){throw e;}console.log(JSON.stringify({tgg_heartbeat_ok:true}));if(!ownerSession?.access_token && !tggCoreEnabled()){await restoreOwnerSessionFromRefreshToken();}console.log(JSON.stringify({tgg_claim_start:true,session_present:Boolean(ownerSession?.access_token),native_tgg_mode:tggCoreEnabled()}));const claim=await Promise.race([tggCoreEnabled()?tggWorkerClaim():rpc.rpc('tgg_browser_cert_worker_claim',{p_worker_id:workerId,p_token:workerToken,p_lease_seconds:300}),new Promise((_,reject)=>setTimeout(()=>reject(new Error('WORKER_CLAIM_TIMEOUT')),15000))]);console.log(JSON.stringify({tgg_claim_result:true,ok:!claim?.error,has_data:Boolean(claim?.data)}));if(claim.error)throw claim.error;let job=tggCoreEnabled()?claim?.job:extractClaimJob(claim.data);console.log(JSON.stringify({tgg_claim_job:true,has_job:Boolean(job),job_id_present:Boolean(job?.id),flow_key:job?.flow_key||null}));if(!job?.id&&tggCoreEnabled()){try{const recovered=await tggWorkerRecoverCertification();job=recovered?.job||null;console.log(JSON.stringify({tgg_certification_recovery:true,ok:true,has_job:Boolean(job),recovered:Boolean(recovered?.recovered),reason:recovered?.reason||null}));if(!job){try{const status=await tggWorkerCertificationStatus();console.log(JSON.stringify({tgg_certification_status:true,counts:status?.counts||[],latest:(status?.latest||[]).map(x=>({id:x.id,status:x.status,certification_type:x.certification_type}))}));}catch(e){console.error(JSON.stringify({tgg_certification_status:true,ok:false,error:e?.message||String(e)}));}}}catch(e){console.error(JSON.stringify({tgg_certification_recovery:true,ok:false,error:e?.message||String(e)}));}}if(!job?.id){last={status:'idle',updated_at:new Date().toISOString()};return;}last={status:'running',flow_key:job.flow_key,job_id:job.id,updated_at:new Date().toISOString()};await viewerEmit(job.browser_session_id||job.payload?.browser_session_id,'job_running',{flow_key:job.flow_key,job_id:job.id});try{if(tggCoreEnabled() && !job.tgg_core_access_token && (job.browser_session_id||job.payload?.browser_session_id)){job.tgg_core_access_token=(await tggWorkerGetBrowserCredential(job.browser_session_id||job.payload.browser_session_id)).access_token;}const result=await Promise.race([job.flow_key==='protected_audio_runtime'?runProtectedAudio(job):job.flow_key==='recording_studio_runtime'?runRecordingStudio(job):runGeneric(job),new Promise((_,reject)=>setTimeout(()=>reject(new Error('BROWSER_FLOW_TIMEOUT')),90000))]);await complete(job,'passed',result,result.evidence||result.captures||[]);last={status:'passed',flow_key:job.flow_key,job_id:job.id,updated_at:new Date().toISOString()};await viewerEmit(job.browser_session_id||job.payload?.browser_session_id,'job_passed',{job_id:job.id});}catch(e){const failure={error:e.message||String(e),real_browser_attempt:true,at:new Date().toISOString()};await complete(job,'failed',failure,[{type:'browser_failure',sha256:sha256(JSON.stringify(failure)),artifact_uri:`tgg://browser-cert/${job.id}/failure.json`,metadata:failure}]);last={status:'failed',flow_key:job.flow_key,job_id:job.id,error:failure.error,updated_at:new Date().toISOString()};await viewerEmit(job.browser_session_id||job.payload?.browser_session_id,'job_failed',{job_id:job.id,error:failure.error});}}catch(e){last={status:'worker_error',error:e.message||String(e),updated_at:new Date().toISOString()};}finally{running=false;}}
