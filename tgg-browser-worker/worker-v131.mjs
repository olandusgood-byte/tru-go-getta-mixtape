...
async function ensureWorkerToken(){
  ...
  if(!workerId||!bootstrapSecret||!SUPABASE_KEY)return false;
  const {data,error}=await rpc.rpc('tgg_browser_cert_worker_bootstrap',{p_worker_id:workerId,p_bootstrap_secret:bootstrapSecret});
  if(error||!data?.ok||!data?.worker_token){console.error(JSON.stringify({tgg_worker_token_recovery:true,ok:false,error:error?.message||'bootstrap_response_invalid'}));return false;}
  workerToken=data.worker_token; process.env.TGG_WORKER_TOKEN=workerToken; globalThis.__TGG_WORKER_TOKEN=workerToken;
  console.log(JSON.stringify({tgg_worker_token_recovery:true,ok:true})); return true;
}
