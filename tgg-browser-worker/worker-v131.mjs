...
import { tggCoreEnabled, tggWorkerHeartbeat, tggWorkerClaim, tggWorkerRecoverCertification, tggWorkerComplete, tggStoreOwnerRefreshToken, tggRestoreOwnerRefreshToken, tggWorkerGetBrowserCredential, tggWorkerBootstrap, tggWorkerRegister, tggEnsureCertification, tggEnsureProtectedAudioCertification } from './tgg-core-client.mjs';
...
let lastEnsureCertificationAt=0;
let recoveredJobForLoop=null;
...
app.get('/enroll', (_req, res) => {
  ...
  const html = [
    ...
    '<script>(function(){const sleep=ms=>new Promise(r=>setTimeout(r,ms));document.getElementById("go").addEventListener("click",async function(){const out=document.getElementById("out"),btn=this;btn.disabled=true;try{const email=document.getElementById("email").value.trim(),password=document.getElementById("password").value;if(!email||!password)throw new Error("Enter owner email and password.");out.textContent="Starting secure enrollment...";const start=await fetch("/enroll/start",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({email,password})});const raw=await start.text();let body={};try{body=JSON.parse(raw)}catch{}if(!start.ok||!body.id)throw new Error(body.error||("Enrollment start HTTP "+start.status));document.getElementById("password").value="";for(let i=0;i<120;i++){await sleep(2000);const r=await fetch("/enroll/status/"+encodeURIComponent(body.id),{cache:"no-store"});const s=await r.json();if(!r.ok){if(r.status===404&&s.error==="enrollment_not_found"){throw new Error("ENROLLMENT_RESTART_REQUIRED")}throw new Error(s.error||("Enrollment status HTTP "+r.status));}if(s.status==="complete"){const result=s.result||{};out.textContent=result.session_persisted===false?"Worker enrolled; session active and persistence retrying. TGG certification started.":"Worker enrolled and browser session bootstrapped. Refresh session persisted. TGG certification started.";return;}if(s.status==="failed")throw new Error((s.error||"Enrollment failed")+(s.detail?": "+s.detail:""));out.textContent="Enrollment in progress — "+(s.stage||"working")+"...";}throw new Error("Enrollment is still processing. Keep this page open and press Enroll Worker once more to re-check.");}catch(e){if((e.message||String(e))==="ENROLLMENT_RESTART_REQUIRED"){out.textContent="Worker was updated while enrolling. Restarting enrollment...";btn.disabled=false;setTimeout(()=>btn.click(),600);return;}out.textContent="ERROR: "+(e.message||String(e));}finally{btn.disabled=false}})})();</script></body></html>'
  ].join('');
