...
app.get('/enroll', (_req, res) => { res.set('Cache-Control','no-store, no-cache, must-revalidate'); res.set('Pragma','no-cache'); const html='<!doctype html><html><head><meta charset="utf-8"><title>TGG Browser Worker Enrollment</title><style>body{font-family:Arial;background:#080808;color:#fff;max-width:720px;margin:50px auto;padding:24px}button{background:#e50914;color:#fff;border:0;padding:12px 18px;border-radius:8px;font-weight:700}input{display:block;width:100%;margin:8px 0;padding:12px;background:#151515;color:#fff;border:1px solid #333;border-radius:8px;box-sizing:border-box}pre{white-space:pre-wrap;background:#111;padding:15px;border-radius:8px}</style></head><body><h1>TGG Self-Hosted Browser Worker</h1><p>Owner-only enrollment. Your authenticated session is handed directly to this worker; worker credentials never leave the server.</p><input id="email" type="email" placeholder="Owner email"><input id="password" type="password" placeholder="Owner password"><button id="go">Enroll Worker</button><pre id="out">Waiting…</pre><script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script><script>window.__TGG_SUPABASE_URL__='+JSON.stringify(SUPABASE_URL)+';window.__TGG_SUPABASE_KEY__='+JSON.stringify(SUPABASE_KEY)+';</script><script>(async function(){const S=window.supabase.createClient(window.__TGG_SUPABASE_URL__,window.__TGG_SUPABASE_KEY__);document.querySelector('#go').onclick=async function(){const out=document.querySelector('#out');try{const email=document.querySelector('#email').value.trim(),password=document.querySelector('#password').value;out.textContent='Signing in…';const a=await S.auth.signInWithPassword({email,password});if(a.error)throw a.error;const sess=a.data&&a.data.session;if(!sess)throw new Error('No authenticated session returned.');out.textContent='Sending owner session…';const b=await fetch('/enroll/session',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({access_token:sess.access_token,refresh_token:sess.refresh_token})});const bj=await b.json();if(!b.ok)throw new Error((bj.error||'Enrollment failed')+(bj.stage?':'+bj.stage:''));out.textContent='Worker enrolled and browser session bootstrapped. Refresh session persisted. TGG certification started.'}catch(e){out.textContent='ERROR: '+(e.message||String(e))}};})();</script></body></html>'; return res.type('html').send(html); });
