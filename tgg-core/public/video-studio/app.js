...
function markDirty(){state.dirty=true;$('#saveState').classList.add('syncing');$('#saveState span').textContent='Unsaved changes';clearTimeout(autosaveTimer);autosaveTimer=setTimeout(function(){saveLocal(true);},700);if(state.cloudUser){clearTimeout(state.cloudAutosaveTimer);state.cloudAutosaveTimer=setTimeout(function(){saveCloud().catch(function(){});},1800);}}
...
function drawClipLayer(c,w,h){
  ...
  if(fx.bloom){
    ctx.save();ctx.globalAlpha*=Math.min(.55,.12+fx.bloom/180);ctx.filter='blur('+(4+fx.bloom*.12)+'px) brightness(145%)';ctx.globalCompositeOperation='screen';ctx.drawImage(compositeSource,0,0);ctx.restore();
  }
  if(fx.rgbSplit){
    var shift=Math.max(1,Math.round(fx.rgbSplit*.09));
    ctx.save();ctx.globalCompositeOperation='screen';ctx.globalAlpha*=.15;ctx.drawImage(compositeSource,-shift,0);ctx.drawImage(compositeSource,shift,0);ctx.restore();
  }
